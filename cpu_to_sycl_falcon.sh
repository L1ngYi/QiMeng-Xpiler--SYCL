...
CPU_SRC_DIR="$BENCH_DIR/cpp_baseline_gemm"
...
echo "=========================================="
echo "Source directory: $CPU_SRC_DIR"
...
# Loop over all .cpp files in the source directory
for src_file in "$CPU_SRC_DIR"/*.cpp; do
    # Check if there are any .cpp files (if the glob doesn't expand, src_file will be the pattern)
    if [ ! -f "$src_file" ]; then
        echo "No .cpp files found in $CPU_SRC_DIR"
        break
    fi
